...
class DropDownPicker extends React.Component {
  constructor(props) {
    ...
    if (!props.multiple) {
      if (props.defaultValue || props.defaultValue === 0) {
        choice = props.items.find(item => item.id === props.defaultValue);
      } else if (
        props.items.filter(
      ...
      defaultValueIndex = props.items.findIndex(
        item => item.id === props.defaultValue,
      );
    } else {
      if (
        props.defaultValue &&
        ...
        props.defaultValue.forEach((id, index) => {
          items.push(props.items.find(item => item.id === id));
        });
      } else if (
        props.items.filter(
          item => item.hasOwnProperty('selected') && item.selected === true,
    ...
    this.state = {
      choice: props.multiple
        ? items
        : {
            record_type: choice !== undefined ? choice.record_type : null,
            id: choice !== undefined ? choice.id : null,
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    if (
      ...
      if (
        ...
        props.defaultValue.forEach((id, index) => {
          items.push(props.items.find(item => item.value === id));
        });
      }
            ...
            (async () => {
              for (const value of defaultValue) {
                await new Promise((resolve, reject) => {
                  resolve(
                    this.select(items.find(item => item.id === id)),
                  );
                });
              }
            })();
          } else {
            this.select(items.find(item => item.id === defaultValue));
          }
        } else {
          this.reset();
        }
  ...
  removeItem(id, {changeDefaultValue = true} = {}) {
    ...
    if (this.state.props.multiple) {
      (async () => {
        for (const id of defaultValue) {
          const item = this.props.items.find(id => item.id === id);
          if (item) {
            await new Promise((resolve, reject) => {
      ...
      const item = this.props.items.find(item => item.id === defaultValue);
  ...
  select(item) {
    ...
    if (!multiple) {
      ...
      const exists = choice.findIndex(
        i => i.record_type === item.record_type && i.id === item.id,
      );

      if (exists > -1 && choice.length > this.props.min) {
        choice = choice.filter(i => i.record_type !== item.record_type && i.id !== item.id);
  ...
  getItems(parent = false) {
    ...
    if (this.state.searchableText) {
      ...
      return items.filter(item => {
        return item.record_type && item.record_type.toLowerCase().indexOf(text) > -1;
      });
    }
  ...
  isSelected(item) {
    return this.state.choice.findIndex(a => a.id === item.id) > -1;
  }
  ...
  getLabel(item, selected = false) {
    ...
    let len2 = record_type.length;
    return record_type + (len !== len2 ? '...' : '');
  }
  ...
  renderItem(item, index, itemsLength) {
    ...
    const isSelected = this.state.choice.id === item.id;
  ...
  render() {
    ...
    const isPlaceholderActive =
      this.state.choice.record_type === null ||
      (Array.isArray(this.state.choice) && this.state.choice.length === 0);
    const record_type = isPlaceholderActive
      ? placeholder
      : this.getLabel(this.state.choice?.record_type, true);
        ...
        <TouchableOpacity
          ...
          <Text
            style={[
              ...
              this.props.labelStyle,
              this.state.choice.record_type !== null && this.props.selectedLabelStyle,
              placeholderStyle,
            ]}
            {...this.props.labelProps}>
            {multiple
